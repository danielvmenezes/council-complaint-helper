...
const CHAT_TABLE_ID = process.env.CHAT_TABLE_ID || "complaint_assistant";
const ACTION_TABLE_ID = process.env.ACTION_TABLE_ID || "complaint_classifier";
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Step 1: Classify the complaint using Action Table
    ...
    if (classifyResponse.ok) {
      ...
      classification = {
        issue_type: classifyData.rows?.[0]?.columns?.issue_type?.choices?.[0]?.message?.content || "unknown",
